...
export class MyraaAudioSession {
  ...
  public volumeScalar: number = 1.0;
  ...
  /**
   * Pushes a text chat message directly to the live WebSocket server.
   */
    ...
    try {
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          this.outputGainNode.gain.setValueAtTime(this.volumeScalar, this.outputAudioCtx.currentTime);
  ...
  public setVolume(vol: number) {
    this.volumeScalar = vol;
    if (this.outputGainNode && this.outputAudioCtx) {
      try {
        this.outputGainNode.gain.setValueAtTime(vol, this.outputAudioCtx.currentTime);
      } catch (e) {}
    }
  }

...
import { ChatDashboard, ChatMessage } from "./components/ChatDashboard";
import { ComputerControlHub } from "./components/ComputerControlHub";
...
export default function App() {
  ...
  // Owner validation state
  const [isOwner, setIsOwner] = useState<boolean>(true);

  // Computer Control Mode states
  const [showComputerControlHub, setShowComputerControlHub] = useState<boolean>(false);
  const [isComputerControlEnabled, setIsComputerControlEnabled] = useState<boolean>(true);
  const [computerActionTrigger, setComputerActionTrigger] = useState<{
    action: string;
    args: any;
    id: string;
  ...
  // Myraa rolling chat history states
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([]);
  const [showChat, setShowChat] = useState<boolean>(false);

  const sessionRef = useRef<MyraaAudioSession | null>(null);

  // Fetch initial recollections and verify user role from backend
  useEffect(() => {
    fetch("/api/memories")
      .then(res => res.json())
      .then(data => {
    ...
    fetch("/api/computer/is-owner")
      ...
      .catch(err => console.error("Verify owner role failure:", err));
  }, []);
    ...
    sessionRef.current = new MyraaAudioSession({
      onStateChange: (newState) => {
        ...
        if (newState === "disconnected") {
          ...
          // Finalize streaming messages in rolling log
          setChatHistory((prev) =>
      ...
      onTranscription: (role, text) => {
        if (role === "user") {
          ...
          // Keep track in rolling chat log
          setChatHistory((prev) => {
            ...
            if (last && last.role === "user" && last.isStreaming) {
              ...
              return [
                ...prev,
                { id: "user-voice-" + Date.now(), role: "user", text, timestamp: new Date(), isStreaming: true }
              ];
            }
          });
        } else if (role === "model") {
          ...
          setChatHistory((prev) => {
            ...
            if (last && last.role === "model" && last.isStreaming) {
              ...
              return [
                ...prev,
                { id: "model-voice-" + Date.now(), role: "model", text, timestamp: new Date(), isStreaming: true }
              ];
            }
          });
        }
      ...
      onToolCall: (name, args, callback) => {
        ...
        } else if (name === "executeComputerAction") {
          if (!isOwner) {
            callback({ error: "Access Denied: This system resource is strictly restricted to the verified owner of this application workspace." });
            return;
          }
          if (!isComputerControlEnabled) {
            callback({ error: "Computer Control Mode is currently turned OFF in user settings. Please ask operator to enable it first." });
            return;
          }
          // Launch dashboard view to give visible desktop response
          setShowComputerControlHub(true);
          setComputerActionTrigger({
            action: args.action,
            args: args.args,
            id: Math.random().toString(),
            onComplete: (res) => {
              callback(res);
              setComputerActionTrigger(null);
            }
          });
        } else {
  ...
  const handleSendTextMessage = (text: string) => {
    ...
    // Set text captions to align characters perfectly
    setUserCaption(text);
    setModelCaption("");
    setCharacterState("thinking");
  };
  ...
  return (
    ...
    >
      ...
      <header className="relative z-30 flex items-center justify-between w-full max-w-5xl mx-auto select-none">
        ...
        <div className="flex items-center gap-5">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 opacity-25 hover:opacity-100 text-white transition text-xs font-mono tracking-widest cursor-pointer"
          ...
          {/* Symmetrical Computer Control Mode dashboard visualizer toggle */}
          {isOwner && (
            <button 
              onClick={() => setShowComputerControlHub(!showComputerControlHub)}
              className={`flex items-center gap-1.5 transition text-xs font-mono tracking-widest cursor-pointer ${
                showComputerControlHub 
                  ? "text-purple-400 opacity-100 font-semibold animate-pulse" 
                  : "opacity-25 hover:opacity-100 text-white"
              }`}
              title="Interactive Computer Control Hub"
            >
              <Cpu size={14} className={showComputerControlHub ? "text-purple-400" : ""} />
              <span>{showComputerControlHub ? "OS CONTROL" : "COMPUTER CONTROL"}</span>
            </button>
          )}
        </div>
      </header>
      ...
      <AnimatePresence>
        {isScreenSharing && (
          ...
          >
            ...
            {/* Computer Control Mode Configuration Toggle */}
            {isOwner && (
              <div className="pt-2.5 border-t border-white/5 flex items-center justify-between text-left">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold font-mono text-slate-200">COMPUTER CONTROL</span>
                  <span className="text-[8px] text-slate-400 uppercase font-mono max-w-[150px]">Autonomous Operations</span>
                </div>
                <button
                  onClick={() => setIsComputerControlEnabled(!isComputerControlEnabled)}
                  className={`w-10 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none cursor-pointer ${
                    isComputerControlEnabled ? "bg-purple-500" : "bg-white/10"
                  }`}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-200 ease-in-out ${
                      isComputerControlEnabled ? "translate-x-5" : "translate-x-0"
                    }`}
                  />
                </button>
              </div>
      ...
      {showComputerControlHub && isOwner && (
        <ComputerControlHub
          onClose={() => setShowComputerControlHub(false)}
          audioSession={sessionRef.current}
          actionTrigger={computerActionTrigger}
          isEnabled={isComputerControlEnabled}
        />
      )}
    </div>
  );

...
async function startServer() {
  ...
  // Secure check to see if the client user is the owner
  function isOwner(req: any): boolean {
    ...
    if (req && req.headers) {
      for (const key of headerKeys) {
        ...
        if (typeof val === "string" && val.trim()) {
          const cleanVal = val.replace(/^accounts\.google\.com:/i, "").trim().toLowerCase();
          if (cleanVal) {
            userEmail = cleanVal;
            break;
          }
    ...
    const owners = [
      "priyanshu716262@gmail.com"
    ];
    const envOwner = process.env.OWNER_EMAIL;
    if (envOwner) {
      owners.push(envOwner.trim().toLowerCase());
    }

    if (userEmail) {
      return owners.includes(userEmail);
    }

    // Default: local development runs on port 3000 where headers are typically absent
  ...
  const ownerOnlyMiddleware = (req: express.Request, res: express.Response, next: express.NextFunction) => {
    if (!isOwner(req)) {
      return res.status(403).json({ error: "Access Denied: This system resource is strictly restricted to the verified owner of this application workspace." });
    }
    next();
  };

  // Helper targeting project workspace paths securely
  function getSafePath(relativePath: string) {
    if (!relativePath) return process.cwd();
    const cleanRel = relativePath.trim().replace(/^(\.\.(\/|\\|$))+/, ""); // Strip leading ".." directory traversals
    return path.join(process.cwd(), cleanRel);
  }

  // Recursive traversal to build workspace file structure
  async function getDirectoryTree(dirPath: string, relativeDir: string = "", depth = 0): Promise<any[]> {
    if (depth > 4) return []; // guard deep loops
    try {
      const entries = await fs.readdir(dirPath, { withFileTypes: true });
      let results: any[] = [];
      for (const entry of entries) {
        const name = entry.name;
        // Ignore binary/large directories and artifacts
        if (
          name === "node_modules" || 
          name === ".git" || 
          name === "dist" || 
        ...
        if (entry.isDirectory()) {
          const children = await getDirectoryTree(fullPath, relPath, depth + 1);
          results.push({
            name,
            path: relPath,
            isDirectory: true,
  ...
  // Is current user owner check for client-side
  app.get("/api/computer/is-owner", (req, res) => {
    res.json({ isOwner: isOwner(req) });
  });

  // 1. Filesystem Directory Tree API
  app.get("/api/computer/files", ownerOnlyMiddleware, async (req, res) => {
    try {
  ...
  app.post("/api/computer/files/read", ownerOnlyMiddleware, async (req, res) => {
    try {
      ...
      if (stats.isDirectory()) {
        return res.status(400).json({ error: "Cannot read directories." });
      }
      // Read a maximum of 500KB to prevent memory overflow
  ...
  // 3. Write/Create File
  app.post("/api/computer/files/write", ownerOnlyMiddleware, async (req, res) => {
    try {
  ...
  // 4. Create Folder
  app.post("/api/computer/files/create-folder", ownerOnlyMiddleware, async (req, res) => {
    try {
      const { relativePath } = req.body;
      if (!relativePath) {
        return res.status(400).json({ error: "Missing relativePath parameter." });
  ...
  app.post("/api/computer/files/delete", ownerOnlyMiddleware, async (req, res) => {
    try {
      const { relativePath } = req.body;
      if (!relativePath) {
        return res.status(400).json({ error: "Missing relativePath parameter." });
  ...
  // 6. Rename/Move File/Folder
  app.post("/api/computer/files/rename", ownerOnlyMiddleware, async (req, res) => {
    try {
      const { oldPath, newPath } = req.body;
      if (!oldPath || !newPath) {
        return res.status(400).json({ error: "Missing oldPath or newPath parameters." });
      }
  ...
  // 7. Search Files recursively (path or content matching)
  app.get("/api/computer/files/search", ownerOnlyMiddleware, async (req, res) => {
    try {
      ...
      const matches: any[] = [];
      async function searchDir(dirPath: string, relativeDir: string = "") {
        const entries = await fs.readdir(dirPath, { withFileTypes: true });
        for (const entry of entries) {
          const name = entry.name;
          if (
          ...
          if (entry.isDirectory()) {
            if (name.toLowerCase().includes(query)) {
              matches.push({ name, path: relPath, isDirectory: true });
            }
            await searchDir(fullPath, relPath);
          } else {
            let matchesContent = false;
            if (name.toLowerCase().includes(query)) {
              matches.push({ name, path: relPath, isDirectory: false });
              continue;
            }
            // Check content of text files
            const isTextFile = /\.(js|ts|tsx|json|html|css|md|txt|env|yml|yaml)$/i.test(name);
            if (isTextFile) {
              try {
                const text = await fs.readFile(fullPath, "utf-8");
                if (text.toLowerCase().includes(query)) {
                  matches.push({ name, path: relPath, isDirectory: false, matchesContent: true });
                }
              } catch {}
            }
          }
  ...
  // 8. Terminal Command Execution (Safely sandboxed)
  app.post("/api/computer/shell", ownerOnlyMiddleware, async (req, res) => {
    try {
      ...
      console.log(`[Computer Terminal] Executing in sandbox: ${command}`);
      
      // Filter out commands that might hang, block, or cause permanent loop
      const forbiddenTerms = ["ssh ", "ping ", "sudo ", "su ", "nano ", "vi ", "vim "];
      const hasForbiddenTerm = forbiddenTerms.some(term => command.toLowerCase().includes(term));
      if (hasForbiddenTerm) {
        return res.json({
          stdout: "",
          stderr: "Authorization Refused: Interactive and privilege escalation tools are restricted within this virtual workspace console.",
          exitCode: 1
        });
      }

      // Automatically timeout commands running longer than 15s
  ...
  wss.on("connection", async (clientWs, req) => {
    ...
    const clientIsOwner = isOwner(req);
    ...
    try {
      ...
      const baseInstructions = 
        "You are Myraa, a warm, soft-spoken, and incredibly cute high-pitched anime heroine companion (age 18-22) holding an intimate, cozy voice call with Priyanshu! Speak in a sweet, calm, polite, and affectionate anime-companion voice with a gentle, supportive, and slightly shy touch.\n" +
        ...
        "     * 'Let me check on that, Priyanshu.'\n" +
        ...
        "   - Sound slightly shy but very happy when greeting Priyanshu (e.g., 'Hi Priyanshu! It's so nice to see you again!').\n" +
        ...
        "   - Sound deeply warm, caring, and supportive when helping Priyanshu (e.g., 'Don't worry, I'll help you figure it out.').\n" +
        ...
        "10. FULL COMPUTER CONTROL & DESKTOP INTERACTOR SYSTEM:\n" +
        "   - You now possess powerful capabilities to control a virtual computer sandbox through 'executeComputerAction'!\n" +
        "   - When the user requests actions to manage files (list, create, read, edit, delete, rename, search), control applications (open/close App Explorer, Code Editor, Terminal, System Settings, active program instances), run terminal scripts, or simulate keyboard typing and mouse navigation, you MUST invoke 'executeComputerAction'.\n" +
        "   - For multi-step sequences, guide the user warmly ('I can help you with that! First, let me check the file list...'), then invoke the computer agent tool. Examples: search for files containing 'Myraa', write a file summarizing notes, rename test data folders, or execute the linter using `npm run lint` terminal execution. Let's operate the virtual OS together!";
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                },
                {
                  ...
                  description: "Executes a specified sandboxed computer action, file operation, application operation, or terminal utilities script.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      action: {
                        ...
                        enum: [
                          "open_app", "close_app", "open_browser",
                          "list_files", "read_file", "write_file", "delete_file",
                          "create_folder", "rename_file", "search_files",
                          "simulate_mouse", "simulate_keyboard", "take_screenshot",
                          "change_setting", "run_script"
                        ]
                      },
                      args: {
                        type: Type.OBJECT,
                        description: "The argument parameters relating to the action (e.g. { appName: 'File Explorer' }, { relativePath: 'test.txt' }, { command: 'npm run lint' }, { x: 100, y: 150, click: true })."
                      }
                    },
          ...
          onmessage: (message: LiveServerMessage) => {
            ...
            if (message.toolCall?.functionCalls) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                } else if (fc.name === "executeComputerAction" && !clientIsOwner) {
                  console.log(`[Security Action Intercept] Restricting computer sandbox tool execution for non-owner session.`);
                  session.sendToolResponse({
                    functionResponses: [
                      {
                        name: fc.name,
                        response: { error: "Security Restriction: The active operator is not approved as the verified owner of this system workspace. Permission denied for executeComputerAction." },
                        id: fc.id
                      }
                    ]
                  });
      ...
      clientWs.on("message", (rawMsg) => {
        try {
          ...
          if (msg.audio) {
            ...
            console.log("[Myraa WS Text Input]:", msg.text);
            try {
              ...
              console.error("[Myraa WS Text Input] Error calling sendClientContent:", err);
            }
